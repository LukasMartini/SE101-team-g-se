...
class MotionModel(modelclass.ModelClass):
    ...
    def process(self, newFrame):
        ...
        self.ofGrey = cv.cvtColor(self.oldFrame, cv.COLOR_BGR2GRAY)  # sets the input frames to greyscale if they weren't already.
        self.nfGrey = cv.cvtColor(newFrame, cv.COLOR_BGR2GRAY)
        self.newPoint, status, error = cv.calcOpticalFlowPyrLK(self.ofGrey, self.nfGrey, self.oldPoint, None, **self.LukasKanadePresets)  # calculates the new optical flow points.

        if self.newPoint is not None:  # takes the array of points returned by calcOpticalFlowPyrLK and only keeps the ones where the status is true (i.e the "best" points)
